...
# from operator_evaluator_sup_elem import *
...
class Compositional(PhysicsBase):
    def __init__(self, property_container, timer, n_points, min_p, max_p, min_z, max_z, min_t=-1, max_t=-1, thermal=0,
        ...
        min_z_list = min_z
        max_z_list = max_z
        if isinstance(min_z, tuple) and isinstance(max_z, tuple):
            min_z_list = list(min_z)
            max_z_list = list(max_z)
        if isinstance(min_z, np.ndarray) and isinstance(max_z, np.ndarray):
            min_z_list = min_z.tolist()
            max_z_list = max_z.tolist()
        if not isinstance(min_z_list, list) and not isinstance(max_z_list, list):
            min_z_list = [min_z] * (self.nc - 1)
            max_z_list = [max_z] * (self.nc - 1)

        if thermal:
            self.vars = ['pressure'] + self.components[:-1] + ['temperature']
            self.n_axes_min = value_vector([min_p] + min_z_list + [min_t])
            self.n_axes_max = value_vector([max_p] + max_z_list + [max_t])
            ...
            self.n_axes_min = value_vector([min_p] + min_z_list)
            self.n_axes_max = value_vector([max_p] + max_z_list)
