...
filename = "obl_res_study_geosx.pdf"
...
axis_names = [['$z_{CO_2}$ [-]', '$z_{H_2O}$ [-]', '$z_{ions}$ [-]'],
              ...
              ['$p$ [bar]', '$s_g$ [-]', '$\phi$ [-]']]
file_name_darts_output = lambda obl_pts: f'results_chemical_benchmark_darts_{obl_pts}.npy'
...
obl_res = np.array([16, 32, 64, 128, 256, 512])
colors = ['b', 'g', 'cyan', 'blue', 'grey', 'purple']
lineType = ['-', '--', '-.', ':', '-', '--']
darts_data_dict = dict()
for ii in obl_res:
    darts_data_dict[ii] = read_darts_output(file_name_darts_output(ii), plot_x, plot_y, index_start, index_stop)
...
for i in range(plot_x):
    for j in range(plot_y):
        ...
        axs[j][i].plot(data_arrays[j][i], 'o',
                       markerfacecolor="None", markeredgecolor='red', markeredgewidth=1, label='GEOSX')
        count = 0
        for k in obl_res:
            axs[j][i].plot(darts_data_dict[k][j][i], colors[count], linewidth=2,
                           label=f'DARTS_{k}', linestyle=lineType[count])
            count += 1
